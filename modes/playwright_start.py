...
def start(url, user, slack_webhook_url, headers, response_url):
    out = subprocess.Popen(['node', '/Users/mukthy/Desktop/office/node-projects/playwrite.js', f'{url}', f'{user}.png'],
        ...
        "blocks": [
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"@{user} Zyte Smartproxy Playwright screenshot for {url} is given below: \n",
                },
            },
            # {
            #     "type": "section",
    ...
    playwright_resp = requests.post(
        url=slack_webhook_url,
        headers=headers,
        data=json.dumps(playwright_result),
    )
    ...
    print(playwright_resp.status_code)
    time.sleep(3)
