...
@app.route("/zytebot-zyteapi-residential", methods=["POST"])
# the below function is to send a response as 200 to slack's post request within 3 sec to avoid the "operation_timed_out" error.
def slack_zytedataapi_residential_response():
    data = request.form
    text = data.get("text")
    validators.url(text)
    user = data.get("user_name")
    ...
    resp = requests.post(response_url, json=message)
    print(resp.status_code)
    zytedataapi_residential_thread = threading.Thread(
        target=zytedataapi_residential, args=(data, text, user, response_url)
    )
    zytedataapi_residential_thread.start()
    return "Processing, Please wait!!"
...
def zytedataapi_isp(data, text, user, response_url):
    print(data)
    print(user)
    ...
    if "firefox" == inputs[0].lower() and len(inputs) > 1:
        ...
        if validators.url(url) is True:
            ...
            # Using a function zyte_api_req from zyte_api module of mode package
            zytedataapi_screenshot_resp = zyte_api_screenshot_firefox_isp.zyte_api_screenshot(
                url, user, slack_webhook_url, headers)
            print(zytedataapi_screenshot_resp)
        ...
        if validators.url(text) is True:
            ...
            # Using a function zyte_api_req from zyte_api module of mode package
            zytedataapi_screenshot_resp = zyte_api_screenshot_isp.zyte_api_screenshot(
                url, user, slack_webhook_url, headers)
            print(zytedataapi_screenshot_resp)
